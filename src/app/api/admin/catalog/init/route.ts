 ...
 *
 * Что делает:
 * 1) Создаёт/обновляет 28 записей в catalog_offers (slug + title)
 * по жёсткому списку CATALOG_SEED ниже.
 * 2) Идёт в Firestore во все 9 приложений (через getLoansCollection),
 * читает офферы, и для каждого документа со slug из CATALOG_SEED
 * создаёт/обновляет строку в catalog_offer_placements
 * (UPSERT по (slug, app_id)).
 * 3) Картинки — в зависимости от параметров:
 * - по умолчанию пытается СКАЧАТЬ картинку из Firestore и
 * ЗАЛИТЬ в Vercel Blob;
 * - при ?useFirestoreImageUrls=1 — просто КОПИРУЕТ ссылку
 * из Firestore в catalog_offers.image_url, без скачивания.
 * Полезно, когда внешний хостинг недоступен с Vercel.
 *
 * Что НЕ делает:
 ...
 *
 * Параметры:
 * ?dryRun=1 — холостой прогон, без записи
 * ?useFirestoreImageUrls=1 — режим B: image_url = ссылка
 * из Firestore (без скачивания)
 *
 * dry-run по-прежнему ничего не пишет, даже с useFirestoreImageUrls.
 */
...
export async function GET(req: NextRequest) {
 ...
 const useFirestoreImageUrls =
 req.nextUrl.searchParams.get('useFirestoreImageUrls') === '1';

 type Report = {
 dry_run: boolean;
 mode: 'blob_upload' | 'firestore_urls_only';
 started_at: string;
 finished_at?: string;
 catalog_offers: { planned: number; written: number };
 placements: {
 planned: number;
 written: number;
 ...
 const report: Report = {
 dry_run: dryRun,
 mode: useFirestoreImageUrls ? 'firestore_urls_only' : 'blob_upload',
 ...
 images: { mapped_offers: 0, uploaded: 0, copied_urls: 0, failed: [], no_source: [] },
 errors: [],
 };

 try {
 // ШАГ 1: catalog_offers (без image_url пока)
 if (!dryRun) {
 ...
 // ШАГ 2: placements + сбор кандидатов на иконку
 ...
 // ШАГ 3: картинки
 if (!dryRun) {
 if (useFirestoreImageUrls) {
 // Режим B: просто копируем URL из Firestore в caталог.
 for (const [slug, srcUrl] of imageCandidates) {
 try {
 const seed = CATALOG_SEED.find((s) => s.slug === slug);
 if (seed) {
 await upsertCatalogOffer(slug, seed.title, srcUrl);
 report.images.copied_urls += 1;
 }
 } catch (e: unknown) {
 ...
 // Дефолт: скачиваем и заливаем в Blob.
 for (const [slug, srcUrl] of imageCandidates) {
