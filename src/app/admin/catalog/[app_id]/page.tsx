 ...
 *
 * Порядок строк: сначала "Добавлен" (по алфавиту title),
 * затем "Не добавлен" (по алфавиту title). Промежуточные
 * статусы (hidden_in_catalog, outside_catalog) идут после
 * "Добавлен" в порядке: hidden_in_catalog → outside_catalog.
 *
...
export default async function CatalogAppPage({
 ...
 const catalogRowsUnsorted: CatalogRow[] = catalogOffers.map((offer) => {
 ...
 // Сортировка: сначала по статусу (placed → hidden_in_catalog →
 // outside_catalog → not_placed), внутри статуса — по title
 // (с учётом локали для корректной сортировки кириллицы).
 const statusOrder: Record<CatalogRow['status'], number> = {
 placed: 0,
 hidden_in_catalog: 1,
 outside_catalog: 2,
 not_placed: 3,
 };
 const catalogRows = catalogRowsUnsorted.slice().sort((a, b) => {
 const so = statusOrder[a.status] - statusOrder[b.status];
 if (so !== 0) return so;
 return a.title.localeCompare(b.title, 'ru');
 });

 // «Вне каталога» — документы Firestore с slug-ом, который НЕ
 // входит в каталог, плюс документы вовсе без slug.
 const outsideRows: OutsideCatalogRow[] = [];
 for (const d of fsAll) {
 if (d.slug && offerBySlug.has(d.slug)) continue;
 outsideRows.push({
 doc_id: d.doc_id,
 slug: d.slug,
 title: d.title,
