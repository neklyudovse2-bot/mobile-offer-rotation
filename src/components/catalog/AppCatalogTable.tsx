 ...
 *
 * Порядок строк определяется на серверной стороне (в page.tsx
 * приложения). Здесь только рендерим в полученной последовательности.
 */
...
function UrlCell({ url }: { url: string | null }) {
 ...
 // Превью: "domain/hash · slug" — slug извлекается из aff_sub3
 // полного URL. Если slug не найден (это не keitaro-ссылка),
 // показываем только "domain/hash" без точки.
 let display = url;
 try {
 const u = new URL(url);
